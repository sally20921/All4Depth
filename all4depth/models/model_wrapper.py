...
from all4depth.datasets.transforms import get_transforms
from all4depth.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from all4depth.utils.horovod import print0, world_size, rank, on_rank_0
from all4depth.utils.image import flip_lr
from all4depth.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from all4depth.utils.logging import pcolor
from all4depth.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics
from all4depth.utils.save import save_depth
from all4depth.models.model_utils import stack_batch
...
def setup_depth_net(config, prepared, **kwargs):
    ...
    depth_net = load_class_args_create(config.name,
        paths=['all4depth.networks.depth',],
        args={**config, **kwargs},
    )
...
def setup_model(config, prepared, **kwargs):
    ...
    model = load_class(config.name, paths=['all4depth.models',])(
        **{**config.loss, **kwargs})
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        if config.dataset[i] == 'KITTI':
            from all4depth.datasets.kitti_dataset import KITTIDataset
        ...
        # DGP dataset
            ...
            from all4depth.datasets.dgp_dataset import DGPDataset
            dataset = DGPDataset(
        ...
        # Image dataset
            ...
            from all4depth.datasets.image_dataset import ImageDataset
